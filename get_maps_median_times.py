...
for i, map_name in enumerate(all_map_names):
    ...
    if map_name in all_map_json_data:
        try:
            if all_map_json_data[map_name]["release"] is None:
                ...
                all_map_json_data[map_name]["release"]
    ...
    try:
        cur_map_data_json = {
            ...
            "fastest_team_finish": raw_data_json["team_ranks"][0]["time"]
            if len(raw_data_json["team_ranks"]) > 0
            else None,
            "fastest_rank_finish": raw_data_json["ranks"][0]["time"]
            if len(raw_data_json["ranks"]) > 0
            else None,
        }
    except Exception:
